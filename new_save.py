...
# ==================== COLUMNS FOR UPDATE (all except match_id) ====================
UPDATE_COLUMNS = [
    'home_id', 'away_id', 'league_id', 'league', 'date',
...
def insert_to_database(db_config, db_name, df):
    """Insert new data and update existing data in a specific database"""
    print(f"\n{'='*80}")
    ...
    if len(df) == 0:
        print(f"\n✓ No records to process for {db_name}.")
        cursor.close()
        conn.close()
        return True
    ...
    if len(existing_data) > 0:
        print(f"\n[4/4] Updating {len(existing_data)} existing records in {db_name}...")
        ...
        for idx, row in existing_data.iterrows():
            try:
                # Build values: map CSV columns to DB columns
                raw_values = [
                    row['home_id'],
                    row['away_id'],
                    row['league_id'],
                ...
                # Convert NaN to None
                update_values = [None if pd.isna(v) else v for v in raw_values]
    ...
    # ==================== INSERT NEW RECORDS ====================
    if len(new_data) > 0:
        print(f"\nInserting {len(new_data)} new records to {db_name}...")
