...
class InferencePipeline:
    def run(self, input_csv):
        ...
        # Map Chinese name
        team_name_id_df = pd.read_csv("map2026.csv")[["TeamId_Agility", "TeamName_Agility"]]
        TEAM_ID_TO_NAME = team_name_id_df.set_index("TeamId_Agility")["TeamName_Agility"].to_dict()
        output['home_team'] = output['home_id'].map(TEAM_ID_TO_NAME)
        output['away_team'] = output['away_id'].map(TEAM_ID_TO_NAME)
        league_name_id_df = pd.read_csv("map2026.csv")[["LeagueID_Agility", "League_Wb"]]
        LEAGUE_ID_TO_NAME = league_name_id_df.set_index("LeagueID_Agility")["League_Wb"].to_dict()
        output['league'] = output['league_id'].map(LEAGUE_ID_TO_NAME)
